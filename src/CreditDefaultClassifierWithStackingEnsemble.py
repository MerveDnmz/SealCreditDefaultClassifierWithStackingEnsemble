...
class CreditDefaultClassifierWithSEAL:
    ...
    def train_stacking_model(self, encrypted=False):
        ...
        if encrypted:
            ...
            self.X_train = self.decrypt_data_in_batches(self.encrypt_data_in_batches(self.X_train, batch_size=256), batch_size=256)
            self.X_test = self.decrypt_data_in_batches(self.encrypt_data_in_batches(self.X_test, batch_size=256), batch_size=256)
        else:
