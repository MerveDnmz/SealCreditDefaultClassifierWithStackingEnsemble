...
class FederatedLearningWithCNN:
    ...
    def train_federated_model(self):
        ...
        local_models = []  # Her bir client için local model ağırlıklarını saklamak
        for client_idx, (X_client, y_client) in enumerate(self.client_data):
            ...
            local_model.fit(X_client, y_client, epochs=5, batch_size=32, verbose=1, class_weight={0: 1, 1: 2})
            local_models.append(local_model)  # Local modeli listeye ekle

        # Global model ağırlıklarını güncelleme
        new_weights = [
            np.mean([local_model.get_weights()[i] for local_model in local_models], axis=0)
